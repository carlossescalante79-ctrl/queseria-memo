...
export default async function handler(req, res) {
  try {
    ...
    if (action === 'createSale') {
      ...
      let status=['paid','partial','pending'].includes(body.paymentStatus) ? body.paymentStatus : 'paid';
      let paid=num(body.paidAmount);
      if (status==='paid') paid=total;
      if (status==='pending') paid=0;
      paid=Math.max(0,Math.min(total,paid));
      if (paid>=total) { paid=total; status='paid'; }
    ...
    if (action === 'createPayment') {
      ...
      const openSales=await sql`
        SELECT id, pending_amount
        FROM sales
        WHERE client_id=${clientId} AND pending_amount>0
        ORDER BY sale_date ASC, id ASC`;

      if (!openSales.length) throw new Error('Este cliente no tiene saldo pendiente.');

      const totalOpen=openSales.reduce((s,r)=>s+num(r.pending_amount),0);
      if (amount>totalOpen) amount=totalOpen;
      ...
      for (const sale of openSales) {
        ...
        const currentPending=num(sale.pending_amount);
        const applied=Math.min(currentPending,remaining);
        const newPending=Math.max(0,currentPending-applied);

        await sql`
          UPDATE sales
          SET pending_amount=${newPending},
        ...
        await sql`
          INSERT INTO payments (client_id,sale_id,payment_date,amount,notes)
          VALUES (${clientId},${Number(sale.id)},${date},${applied},${note})`;

        remaining-=applied;
        appliedTotal+=applied;
      }

      return res.status(200).json({ok:true,applied:appliedTotal});
    }
