...
async function getData(){
  ...
  return {
    ...
    orders:orders.map(o=>({id:Number(o.id),date:dateOnly(o.order_date),providerId:o.provider_id?Number(o.provider_id):null,provider:o.provider_name||'Sin proveedor',notes:o.notes||'',total:num(o.total),items:oi.get(Number(o.id))||[]})),
    sales:sales.map(s=>({id:Number(s.id),date:dateOnly(s.sale_date),clientId:s.client_id?Number(s.client_id):null,client:s.client_name,concept:s.client_name,paymentStatus:s.payment_status,amount:num(s.total),paidAmount:num(s.paid_amount),pendingAmount:num(s.pending_amount),note:s.notes||'',items:si.get(Number(s.id))||[]})),
    payments:payments.map(p=>({id:Number(p.id),clientId:p.client_id?Number(p.client_id):null,saleId:p.sale_id?Number(p.sale_id):null,date:dateOnly(p.payment_date),amount:num(p.amount),note:p.notes||''}))
  };
}
...
export default async function handler(req,res){
  try{
    ...
    if(a==='updateClientProfile'){
      const id=Number(b.id),name=String(b.name||'').trim(),phone=String(b.phone||''),route=['monday','tuesday'].includes(b.route)?b.route:null;
      if(!id||!name) throw new Error('Datos de cliente inválidos.');
      const old=await sql`SELECT name FROM clients WHERE id=${id} LIMIT 1`;
      if(!old[0]) throw new Error('Cliente no encontrado.');
      const duplicate=await sql`SELECT id FROM clients WHERE lower(name)=lower(${name}) AND id<>${id} LIMIT 1`;
      if(duplicate[0]) throw new Error('Ya existe otro cliente con ese nombre.');
      await sql`UPDATE clients SET name=${name},phone=${phone},route=${route},updated_at=NOW() WHERE id=${id}`;
      await sql`UPDATE sales SET client_name=${name},updated_at=NOW() WHERE client_id=${id}`;
    ...
    if(a==='updateOrder'){
      const id=Number(b.id),items=Array.isArray(b.items)?b.items:[];
      if(!id) throw new Error('Pedido inválido.');
      if(!items.length) throw new Error('El pedido debe tener productos.');
      const provider=String(b.provider||'Sin proveedor');
      const providerId=await findProvider(provider);
      const total=items.reduce((s,i)=>s+num(i.qty)*num(i.cost),0);
      await sql`UPDATE orders SET order_date=${String(b.date||'')},provider_id=${providerId},provider_name=${provider},notes=${String(b.notes||'')},total=${total},updated_at=NOW() WHERE id=${id}`;
      await sql`DELETE FROM order_items WHERE order_id=${id}`;
      for(const i of items){
        const productId=await findProductId(i.product);
        await sql`INSERT INTO order_items(order_id,product_id,product_name,quantity_kg,unit_cost,subtotal) VALUES(${id},${productId},${String(i.product)},${num(i.qty)},${num(i.cost)},${num(i.qty)*num(i.cost)})`;
      }
      return res.status(200).json({ok:true});
    }
    if(a==='updateSaleDate'){
      const id=Number(b.id),date=String(b.date||'');
      if(!id||!date) throw new Error('Fecha inválida.');
      await sql`UPDATE sales SET sale_date=${date},updated_at=NOW() WHERE id=${id}`;
      return res.status(200).json({ok:true});
    }
    if(a==='updateSaleAmount'){
      const id=Number(b.id);
      const rawAmount=Number(b.amount);
      if(!id||!Number.isFinite(rawAmount)||rawAmount<0) throw new Error('Monto inválido.');

      const sale=await sql`SELECT paid_amount FROM sales WHERE id=${id} LIMIT 1`;
      if(!sale[0]) throw new Error('Crédito no encontrado.');

      const total=Math.round(rawAmount*100)/100;
      const paid=Math.round(num(sale[0].paid_amount)*100)/100;
      if(total<paid) throw new Error(`El total no puede ser menor a lo que ya se pagó: $${paid.toFixed(2)}.`);

      const pending=Math.max(0,Math.round((total-paid)*100)/100);
      const status=pending<=0?'paid':paid>0?'partial':'pending';
      await sql`UPDATE sales SET total=${total},pending_amount=${pending},payment_status=${status},updated_at=NOW() WHERE id=${id}`;
      return res.status(200).json({ok:true,total,paidAmount:paid,pendingAmount:pending});
    }
